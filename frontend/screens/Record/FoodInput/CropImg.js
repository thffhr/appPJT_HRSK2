...
export default class CropImg extends Component {
  ...
  checkLocation(info) {
    ...
    this.setState({
      location: [info['left'] / width, info['top'] / width, info['right'] / width, info['bottom'] / width,]
    })
  };
  sendLocation() {
  ...
  changeSize(selected) {
    var Size = {'very small': 0.2, 'small': 0.3, 'medium': 0.5, 'large': 0.7, 'very large': 0.9}
    ...
    return (
      <View style={{width: '100%', height: '100%', flex: 1}} >
        <Draggable
            ...
            maxX={width}
            maxY={width}
            onDragRelease={(e, gestureState, getBounds)=>this.checkLocation(getBounds)}
          />
          <Image
